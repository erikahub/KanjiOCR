...
# fulldata = dc.data
# labels, features = [key for key in fulldata.keys() for i in range(len(fulldata[key]))], [val for l in fulldata.values() for val in l]
# print(labels[:4])
...
# (X_train, X_test, y_train, y_test ) = \
    # sklearn.model_selection.train_test_split(features, labels, train_size=0.7, random_state=182)

# print(y_train[:5])
# from PIL import Image
# for x, y in zip(X_train, y_train):
    # plt.figure(figsize=(10,10))
    # plt.imshow(Image.frombytes('F', (64, 63), x, 'bit', 4), cmap='binary')
    # plt.show()
